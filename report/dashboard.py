...
class InlineCSS(BaseComponent):
    """Embed report.css directly inside a <style> tag."""
    def build_component(self, entity_id, model):
        css_file = Path(__file__).resolve().parent.parent / "assets" / "report.css"
...
class Header(BaseComponent):
    ...
    def build_component(self, entity_id, model: QueryBase):
        ...
        return H1(title, class_="report-header")
...
class LineChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model: QueryBase):
        ...
        ax.set_title("Cumulative Performance Events", fontsize=16)
        ax.set_xlabel("Date")
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model: QueryBase):
        ...
        color = cm.get_cmap("RdYlGn_r")(pred)
        # Initialize a matplotlib subplot
        fig, ax = plt.subplots(figsize=(6, 2.5))
        ax.set_ylim(-0.4, 0.8)       # lower, upper bound

        ax.text(pred + 0.02, 0, f"{pred:.1%}",va="center", ha="left", fontsize=11)
...
class Visualizations(CombinedComponent):
    ...
    outer_div_type = Div(class_='grid')
